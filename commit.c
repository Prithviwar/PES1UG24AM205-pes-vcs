...
int commit_create(const char *message, ObjectID *commit_id_out) {
    ...
    // 1) Build root tree from index
    if (tree_from_index(&commit.tree) != 0) return -1;

    // 2) Read parent from HEAD when available (initial commit has no parent)
    ...
    // 4) Serialize + write commit object
    void *raw = NULL;
    size_t raw_len = 0;
    ...
    // 5) Move HEAD/branch to the new commit
